...
public class ClassUtil {
    ...
    public static List<BeanInfo> getPropertyDesc(Class<?> clazz) {
    ...
    public static Map<String,BeanInfo> getMapPropertyDesc(Class<?> clazz) {
        List<?> temp = beanInfos.get(clazz);
        if (null != temp) {
            //return temp;
        }
    ...
    /*
	private static void getPkMockValue(Map<String,PropertyDescriptor> beanInfos, Object obj, List<String> pk) {
        Iterator<String> it = pk.iterator();
        Object[] objs = new Object[pk.size()];
        int i = 0;
        while (it.hasNext()) {
            String name = it.next();
            PropertyDescriptor descriptor = beanInfos.get(name);
            Method readMethod = descriptor.getReadMethod();
            Object value = getMockValue(readMethod.getReturnType());
            objs[i] = value;
            i++;
    ...
    */
    ...
    public static String objectToString(final Object obj) {
    	...
    	while (it.hasNext()) {
    		...
    		builder.append("[").append(bean.getProperty()).append("=");
    		try {
    	...
    	while (it.hasNext()) {
    		...
    		try {
				...
				if (clazz.isPrimitive() || clazz==String.class || clazz==Integer.class
					...
					Map<String, String> temp = getObjectProperties(value);
					Iterator<String> tempId = temp.keySet().iterator();
					while (it.hasNext()) {
						String tempKey = tempId.next();
						String tempValue = temp.get(tempKey);
